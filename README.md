...
Không được đưa các file sau lên GitHub:
...
### 14.1. Lỗi SSH `Permission denied`

Nguyên nhân thường gặp:

- Dùng sai private key.
- EC2 không gắn đúng key pair.
- Dùng sai user.
- Security Group chưa mở SSH từ IP hiện tại.
...
### 14.2. Lỗi không resolve hostname

Ví dụ lỗi:

```text
Could not resolve hostname PUBLIC_EC2_PUBLIC_IP
```

Nguyên nhân: chưa thay `PUBLIC_EC2_PUBLIC_IP` bằng IP thật.
...
### 14.3. Test fail ở Public Subnet

Nếu test báo:

```text
[FAIL] Public subnet maps public IP on launch
```

Kiểm tra trong module subnet:
...
### 14.4. Không SSH được vào Private EC2

Kiểm tra Private EC2 Security Group phải có inbound rule:

```text
SSH TCP 22 source = Public EC2 Security Group
```

Có thể kiểm tra bằng test:
